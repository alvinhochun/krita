...
#include "kis_filter_pixelize_test.h"
#include <QTest>
...
void KisFilterPixelizeTest::testFilterPixelize()
{
    ...
    for (QList<QString>::Iterator it = filterList.begin(); it != filterList.end(); ++it) {
        if (*it != QStringLiteral("pixelize")) continue;
...
void KisFilterPixelizeTest::testFilterPixelizeSrcNotIsDev()
{
    QStringList failures;
    QStringList successes;
...
void KisFilterPixelizeTest::testFilterPixelizeWithSelections()
{
    QStringList failures;
    QStringList successes;
...
QTEST_MAIN(KisFilterPixelizeTest)
